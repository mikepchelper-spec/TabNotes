...
export default function SidePanelApp() {
  ...
  const parseChecklistItems = useCallback((html: string) => {
    if (!html) return [];
  ...
  // Synchronize checklist mode and items when switching notes or updating content
  useEffect(() => {
    if (isUpdatingChecklistRef.current) {
    ...
    const isChecklist = content.trim().startsWith('- [ ]') || content.trim().startsWith('- [x]');
    setChecklistMode(isChecklist);
    if (isChecklist) {
      setChecklistItems(parseChecklistItems(content));
    } else {
      setChecklistItems([]);
    }
  }, [activeNoteId, content, parseChecklistItems]);
                ...
                <div className="sp-note-meta">
                  ...
                  {activeNoteId && (
                    <button
                      ...
                      title={checklistMode ? 'Ocultar casillas (Texto plano)' : 'Mostrar casillas (Estilo Google Keep)'}
                    >
                      {checklistMode ? '☑' : '☐'}
                    </button>
                  )}
